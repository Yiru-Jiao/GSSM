...
def fill_na_warning(results):
    ...
    results['safety_recorded'] = results['safety_recorded'].astype(bool)
...
def main(path_result, path_prepared):
    ...
    # UCD
    if os.path.exists(path_result + 'Conflicts/Results/RiskEval_UCD.h5'):
        print('--- Evaluation with UCD already completed ---')
    else:
        print('--- Evaluating with UCD ---')
        thresholds = np.unique(np.round(10**np.arange(0,5.95,0.055))-2)
        safety_evaluation = read_evaluation('UCD', path_eval)
        progress_bar = tqdm(thresholds, desc='UCD', ascii=True, dynamic_ncols=False, miniters=10)
        records = Parallel(n_jobs=-1)(delayed(evaluate)('UCD', threshold, safety_evaluation, event_data, event_meta, voted_targets) for threshold in progress_bar)
        records = pd.concat(records).reset_index()
        records['indicator'] = 'UCD'
        records['model'] = 'UCD'
        records = fill_na_warning(records)
        records.to_hdf(path_result + 'Conflicts/Results/RiskEval_UCD.h5', key='results', mode='w')
        progress_bar.close()
        print('UCD time elapsed: ' + systime.strftime('%H:%M:%S', systime.gmtime(systime.time() - initial_time)))
