...
def main(args, manual_seed, path_prepared, path_result):
    ...
    # Read events
    warning = pd.read_hdf(path_result+'Analyses/Warning_SafeBaseline_current+acc_environment_profiles_not_pretrained.h5', key='results')
    ...
    for event_id, target_id in tqdm(warning[['event_id','target_id']].values, total=len(warning), ascii=True, desc='Attribution', miniters=50):
