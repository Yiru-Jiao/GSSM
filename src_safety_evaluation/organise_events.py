...
def main(meta_both, events):
    ...
    for event_cat in meta_both['event_category'].value_counts().index.values[::-1]:
        ...
        for event_id in tqdm(event_meta.index.values, desc='Event'):
            ...
            new_vars = ['start', 'end', 'closest', 'reaction']
        ...
        event_ids_to_remove = event_meta[event_meta[['start_timestamp', 'end_timestamp', 'closest_timestamp']].isnull().any(axis=1)].index.values
