...
def draw_map_without_lanelet(filename, axes, lat_origin, lon_origin):
    ...
    for way in e.findall('way'):
        ...
        if way_type is None:
            ...
            if way_subtype == "dashed":
                type_dict = dict(color="white", linewidth=0.5, zorder=10, dashes=[10, 10])
            else:
                type_dict = dict(color="white", linewidth=0.5, zorder=10)
            ...
            type_dict = dict(color="white", linewidth=0.5, zorder=10, dashes=[5, 10])
        elif way_type == "bike_marking":
            type_dict = dict(color="white", linewidth=0.5, zorder=10, dashes=[5, 10])
        elif way_type == "stop_line":
            type_dict = dict(color="white", linewidth=1.5, zorder=10)
            x_list, y_list = get_x_y_lists(way, point_dict)
            x_lists.append(x_list)
            y_lists.append(y_list)
        elif way_type == "virtual":
            type_dict = dict(color="lavender", linewidth=0.5, zorder=10, dashes=[2, 5])
        elif way_type == "road_border":
            type_dict = dict(color="blue", linewidth=0.5, zorder=10)
    ...
    return (x_lists, y_lists, type_dicts)
