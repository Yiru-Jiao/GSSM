...
def main(args, manual_seed, path_prepared):
    ...
    if os.path.exists(path_prepared + 'PosteriorInference/evaluation.csv'):
        ...
        evaluation = pd.DataFrame(columns=['encoder_selection', 'pretraining', 'initial_lr', 'batch_size', 'test_loss'])
        ...
        for encoder_selection in encoder_combinations:
            ...
            for factor in factor_range:
                ...
                if len(evaluation[condition])>0 and evaluation.loc[condition, 'test_loss'].values[0]>0:
                    print(f"Training with encoders: {'_'.join(encoder_selection)}, pretrained: {pretrained_encoder}, initial_lr: {initial_lr}, batch_size: {batch_size} already done.")
                    continue
                print(f"Training with encoders: {'_'.join(encoder_selection)}, pretrained: {pretrained_encoder}, initial_lr: {initial_lr}, batch_size: {batch_size}")
                ...
                if os.path.exists(pipeline.path_output + f'bs={batch_size}-initlr={initial_lr}/val_loss_log.csv'):
                    print(f"Loading trained model: {'_'.join(encoder_selection)}, pretrained: {pretrained_encoder}, initial_lr: {initial_lr}, batch_size: {batch_size}.")
                    pipeline.load_model(batch_size, initial_lr)
                else:
                    pipeline.train_model(epochs, initial_lr, verbose=50)
                ...
                evaluation.loc[len(evaluation)] = ['_'.join(encoder_selection),
                                                   ...
                                                   batch_size,
                                                   test_loss]
                evaluation = evaluation.sort_values(by=['encoder_selection', 'pretraining', 'initial_lr', 'batch_size'])
